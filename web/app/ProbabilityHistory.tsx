...
export default function ProbabilityHistory({ gameId, live }: { gameId: string; live: boolean }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!controller.signal.aborted) { setSamples(payload.snapshots); setError(false); }
  ...
  const x = (s: Snapshot) => 50 + (s.observedAt - firstTime) / span * 890;
  ...
  function paths(key: 'finalTie' | 'overtime') {
    ...
    return samples.map(s => {
      ...
      const move = start || s.observedAt - previousTime > (s.origin === 'reconstructed' ? 600000 : 120000);
  ...
  return <section className="history-panel" aria-label="Recorded game probabilities">
    ...
    {reconstructed && <p className="history-note"><strong>Reconstructed from ESPN play-by-play.</strong> These estimates were calculated after the game from each pre-play state, using the current model. They were not recorded or posted live.</p>}
    <svg className="history-chart" viewBox="0 0 960 230" role="img" aria-label="Final-tie and overtime probability over time; use the slider below to inspect each recorded snapshot">
      ...
      <text x="50" y="219">{new Date(firstTime).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</text>
      <text x="940" y="219" textAnchor="end">{new Date(samples.at(-1)!.observedAt).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' })}</text>
    </svg>
    ...
    <p className="history-note">{samples.length} saved snapshots. Gaps mark unavailable estimates or long breaks. {point.origin === 'reconstructed' ? 'Selected point: reconstructed pre-play estimate.' : 'Selected point: recorded live.'} Final-tie estimates use a drive simulation; saved estimates are never recalculated.</p>
  </section>;
}
